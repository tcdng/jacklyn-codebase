...
/**
 * Default notification business module implementation.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Transactional
@Component(NotificationModuleNameConstants.NOTIFICATIONBUSINESSMODULE)
public class NotificationModuleImpl extends AbstractJacklynBusinessModule
	...
	public NotificationModuleImpl() {
		templates = new FactoryMap<String, MessageTemplateDef>(true) {
			...
			@Override
			protected boolean stale(String globalTemplateName,
					MessageTemplateDef notificationTemplateDef) throws Exception {
				boolean stale = false;
				try {
					TemplateNameParts templateNames
							= NotificationUtils.getTemplateNameParts(globalTemplateName);
					long versionNo = db().value(long.class, "versionNo",
							new NotificationTemplateQuery()
									.moduleName(templateNames.getModuleName())
			...
			@Override
			protected MessageTemplateDef create(String globalTemplateName, Object... params)
					throws Exception {
				TemplateNameParts templateNames
						= NotificationUtils.getTemplateNameParts(globalTemplateName);
				NotificationTemplate notificationTemplate = db().list(
						new NotificationTemplateQuery().moduleName(templateNames.getModuleName())
								.name(templateNames.getTemplateName()));
				if (notificationTemplate == null) {
					throw new UnifyException(
							NotificationModuleErrorConstants.MESSAGE_TEMPLATE_WITH_NAME_UNKNOWN,
							globalTemplateName);
				}
				...
				return new MessageTemplateDef(notificationTemplate.getId(),
						...
						StringUtils
								.breakdownParameterizedString(notificationTemplate.getTemplate()),
		...
		channels = new FactoryMap<String, MessagingChannelDef>(true) {
			...
			@Override
			protected boolean stale(String name, MessagingChannelDef notificationChannelDef)
					throws Exception {
				boolean stale = false;
				try {
					long versionNo = db().value(long.class, "versionNo",
							new NotificationChannelQuery().name(name));
			...
			protected MessagingChannelDef create(String name, Object... params) throws Exception {
				NotificationChannel notificationChannel
						= db().list(new NotificationChannelQuery().name(name));
				if (notificationChannel == null) {
					throw new UnifyException(
							NotificationModuleErrorConstants.MESSAGE_CHANNEL_WITH_NAME_UNKNOWN, name);
				}

				String userName = null;
				String password = null;
				if (notificationChannel.getAuthenticationId() != null) {
					Authentication authentication = systemModule
							.findAuthentication(notificationChannel.getAuthenticationId())
							.getData();
					userName = authentication.getUserName();
					password = authentication.getPassword();
				}

				return new MessagingChannelDef(notificationChannel.getId(),
						notificationChannel.getName(), notificationChannel.getNotificationType(),
						notificationChannel.getHostAddress(), notificationChannel.getHostPort(),
						notificationChannel.getSecurityType(), userName, password,
	...
	@Override
	public Long createNotificationTemplate(NotificationTemplate notificationTemplate)
			throws UnifyException {
		return (Long) db().create(notificationTemplate);
	}

	@Override
	public NotificationTemplate findNotificationTemplate(Long notificationTemplateId)
			throws UnifyException {
		return db().find(NotificationTemplate.class, notificationTemplateId);
	}

	@Override
	public NotificationTemplate findNotificationTemplate(String moduleName, String name)
			throws UnifyException {
		return db().list(new NotificationTemplateQuery().moduleName(moduleName).name(name));
	}

	@Override
	public List<NotificationTemplate> findNotificationTemplates(NotificationTemplateQuery query)
			throws UnifyException {
		return db().listAll(query.select("id", "name", "description", "subject", "htmlFlag",
				"moduleId", "moduleName", "moduleDescription", "status", "statusDesc"));
	}

	@Override
	public int updateNotificationTemplate(NotificationTemplate notificationTemplate)
			throws UnifyException {
	...
	@Override
	public Long createNotificationChannel(NotificationChannel notificationChannel)
			throws UnifyException {
		return (Long) db().create(notificationChannel);
	}

	@Override
	public NotificationChannel findNotificationChannel(Long notificationChannelId)
			throws UnifyException {
		return db().find(NotificationChannel.class, notificationChannelId);
	}

	@Override
	public List<NotificationChannel> findNotificationChannels(NotificationChannelQuery query)
			throws UnifyException {
		return db().listAll(query);
	}

	@Override
	public int updateNotificationChannel(NotificationChannel notificationChannel)
			throws UnifyException {
	...
	public void sendNotification(Message message) throws UnifyException {
		...
		Long notificationTemplateId
				= templates.get(message.getGlobalTemplateName()).getNotificationTemplateId();
		notification.setNotificationTemplateId(notificationTemplateId);

		Long notificationChannelId
				= channels.get(message.getNotificationChannelName()).getNotificationChannelId();
		...
		if (!message.getAttachments().isEmpty()) {
			...
			for (com.tcdng.jacklyn.notification.data.Message.Attachment attachment : message
					.getAttachments()) {
	...
	@Override
	public int setNotificationStatus(List<Long> notificationIds, NotificationStatus status)
			throws UnifyException {
		return db().updateAll(new NotificationQuery().idIn(notificationIds),
				new Update().add("status", status));
	}
	...
	public void sendNotifications(TaskMonitor taskMonitor) throws UnifyException {
		if (grabClusterMasterLock()) {
			if (systemModule.getSysParameterValue(boolean.class,
				...
				int maxBatchSize = systemModule.getSysParameterValue(int.class,
						NotificationModuleSysParamConstants.NOTIFICATION_MAX_BATCH_SIZE);
				int maxAttempts = systemModule.getSysParameterValue(int.class,
						NotificationModuleSysParamConstants.NOTIFICATION_MAXIMUM_TRIES);
				int retryMinutes = systemModule.getSysParameterValue(int.class,
				...
				for (Notification notification : notificationList) {
					...
					MessageDictionary messageDictionary = IOUtils
							.streamFromBytes(MessageDictionary.class, notification.getDictionary());
					if (sendNotification(notification, messageDictionary)) {
						...
						if (attempts >= maxAttempts) {
							...
							Date dueDt = CalendarUtils
									.getNowWithFrequencyOffset(FrequencyUnit.MINUTE, retryMinutes);
	...
	public void installFeatures(List<ModuleConfig> moduleConfigList) throws UnifyException {
		...
		for (ModuleConfig moduleConfig : moduleConfigList) {
			Long moduleId = systemModule.getModuleId(moduleConfig.getName());
			if (moduleConfig.getNotificationTemplates() != null) {
				...
				for (NotificationTemplateConfig notificationTemplateConfig : moduleConfig
						.getNotificationTemplates().getNotificationTemplateList()) {
					mtQuery.clear();
					NotificationTemplate oldNotificationTemplate = db().find(
							mtQuery.moduleId(moduleId).name(notificationTemplateConfig.getName()));
					String description = resolveApplicationMessage(
							notificationTemplateConfig.getDescription());
					if (oldNotificationTemplate == null) {
						...
						oldNotificationTemplate
								.setTemplate(notificationTemplateConfig.getTemplate());
	...
	private boolean sendNotification(Notification notification,
		...
		try {
			MessageTemplateDef notificationTemplateDef = templates
					.get(NotificationUtils.getGlobalTemplateName(notification.getModuleName(),
							notification.getNotificationTemplateName()));
			MessagingChannelDef notificationChannelDef
					= channels.get(notification.getNotificationChannelName());
			...
			messageDictionary.addEntry("senderName",
					resolveApplicationMessage(notification.getSenderName()));
			String messageBody = StringUtils.buildParameterizedString(
					notificationTemplateDef.getTokenList(), messageDictionary.getDictionary());
			...
			if (!StringUtils.isBlank(attachmentGenerator)) {
				...
				fileAttachmentList.addAll(
						notificationAttachmentGenerator.generateAttachments(messageDictionary));
				isAttachment = true;
			}

			// Database attachments
			List<NotificationAttachment> attachmentList = db().listAll(
					new NotificationAttachmentQuery().notificationId(notification.getId()));
			if (!attachmentList.isEmpty()) {
				...
				for (NotificationAttachment notificationAttachment : attachmentList) {
					fileAttachmentList.add(new FileAttachment(notificationAttachment.getType(),
							...
							notificationAttachment.getData()));
				}
			}

			// Send
			return notificationChannel.sendMessage(notificationChannelDef, subject, senderContact,
					recipientContactList, messageBody, notificationTemplateDef.isHtml(),
					fileAttachmentList);
