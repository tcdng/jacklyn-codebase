...
public class NotificationServiceImpl extends AbstractJacklynBusinessService
		...
		channels = new FactoryMap<String, NotificationChannelDef>(true) {
			...
			protected NotificationChannelDef create(String name, Object... params) throws Exception {
				...
				if (notificationChannel.getAuthenticationId() != null) {
					Authentication authentication = systemService
	...
	public List<NotificationTemplate> findNotificationTemplates(NotificationTemplateQuery query) throws UnifyException {
		return db().listAll(query.addSelect("id", "name", "description", "subject", "htmlFlag", "moduleId",
				"moduleName", "moduleDescription", "status", "statusDesc"));
	}
	...
	public void sendNotification(Message message) throws UnifyException {
		...
		if (notifChannelDef.getNotificationType().internal()) {
			...
			notification.setStatus(NotificationStatus.PREPARATION);
			...
			db().updateById(Notification.class, notificationId,
					new Update().add("status", NotificationStatus.NOT_SENT));
		}
	}
	...
	public List<? extends SystemNotification> findUserSystemNotifications(String userId) throws UnifyException {
		List<NotificationInbox> notificationList = db()
				.listAll(new NotificationInboxQuery().userId(userId).addOrder(OrderType.DESCENDING, "createDt"));
		...
		if (grabClusterLock(SEND_NOTIFICATION_LOCK)) {
			try {
				if (systemService.getSysParameterValue(boolean.class,
					...
					int maxBatchSize = systemService.getSysParameterValue(int.class,
							NotificationModuleSysParamConstants.NOTIFICATION_MAX_BATCH_SIZE);
					int maxAttempts = systemService.getSysParameterValue(int.class,
							NotificationModuleSysParamConstants.NOTIFICATION_MAXIMUM_TRIES);
					int retryMinutes = systemService.getSysParameterValue(int.class,
							NotificationModuleSysParamConstants.NOTIFICATION_RETRY_MINUTES);

					Date now = db().getNow();
					List<Notification> notificationList = db().listAll(new NotificationQuery().due(now)
							.status(NotificationStatus.NOT_SENT).orderById().setLimit(maxBatchSize));
					for (Notification notification : notificationList) {
						...
						if (notification.getDictionary() != null) {
							messageDictionary = new MessageDictionary((Map<String, Object>) IOUtils
									.streamFromBytes(Map.class, notification.getDictionary()));
						} else {
						...
						if (sendNotification(notification, messageDictionary)) {
							...
							if (attempts >= maxAttempts) {
								...
								Date dueDt = CalendarUtils.getNowWithFrequencyOffset(now, FrequencyUnit.MINUTE,
										retryMinutes);
	...
	private boolean sendNotification(Notification notification, MessageDictionary messageDictionary) {
		try {
			NotificationTemplateDef notificationTemplateDef = templates.get(NotificationUtils
					.getTemplateGlobalName(notification.getModuleName(), notification.getNotificationTemplateName()));
