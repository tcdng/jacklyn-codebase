...
/**
 * Default archiving module implementation.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Transactional
@Component(ArchivingModuleNameConstants.ARCHIVINGBUSINESSMODULE)
public class ArchivingModuleImpl extends AbstractJacklynBusinessModule implements ArchivingModule {
	...
	@Override
	public List<ArchivableDefinition> findArchivableDefinitions(ArchivableDefinitionQuery query)
	...
	@Override
	public List<ArchivingField> findArchivingFields(ArchivingFieldQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Override
	public List<FileArchiveConfig> findFileArchiveConfigs(FileArchiveConfigQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Taskable(name = FileArchiveTaskConstants.BUILDLOBFILEARCHIVETASK,
			description = "Build LOB File Archive Taskable",
			parameters = { @Parameter(name = FileArchiveTaskParamConstants.FILEARCHIVECONFIGNAME,
					...
					editor = "!ui-select list:filearchiveconfiglist listKey:name blankOption:$s{}",
					mandatory = true),
					@Parameter(name = FileArchiveTaskParamConstants.WORKINGDT, type = Date.class) },
			schedulable = true)
	public Long executeBuildLobFileArchiveTask(TaskMonitor taskMonitor,
			String fileArchiveConfigName, Date workingDt) throws UnifyException {
		Long fileArchiveId = null;
		try {
			FileArchiveConfig fileArchiveConfig
					= db().list(new FileArchiveConfigQuery().name(fileArchiveConfigName));
			...
			query.between(fileArchiveConfig.getDateFieldName(),
					CalendarUtils.getMidnightDate(workingDt),
			...
			if (!targetIdList.isEmpty()) {
				...
				String filename = fileArchiveNameGenerator.generateFileArchiveName(
						FileArchiveType.LOB_FILE_ARCHIVE, fileArchiveConfigName, workingDt);
	...
	@Override
	public byte[] retriveArchivedBlob(String recordName, String fieldName, Long archivedItemId)
			throws UnifyException {
		FileArchiveEntry fileArchiveEntry = db().list(new FileArchiveEntryQuery()
				.recordName(recordName).fieldName(fieldName).archivedItemId(archivedItemId));
		if (fileArchiveEntry != null) {
			String localArchivePath = JacklynUtils.getExtendedFilePath(
					fileArchiveEntry.getLocalArchivePath(),
					fileArchiveEntry.getLocalArchiveDateFormat(), fileArchiveEntry.getArchiveDt());
			String absoluteFilename
					= fileSystemIO.buildFilename(localArchivePath, fileArchiveEntry.getFilename());
			if (fileSystemIO.isFile(absoluteFilename)) {
				// Read from local
				InputStream inputStream = null;
				try {
					inputStream = fileSystemIO.openFileInputStream(absoluteFilename,
							fileArchiveEntry.getFileIndex());
				...
				if (backupFileTransferCfgId != null) {
					return fileModule.readRemoteBlock(backupFileTransferCfgId,
							fileArchiveEntry.getArchiveDt(), fileArchiveEntry.getFilename(),
							fileArchiveEntry.getFileIndex(),
	...
	@Override
	public String retriveArchivedClob(String recordName, String fieldName, Long archivedItemId)
			throws UnifyException {
		try {
