...
/**
 * Dashboard sections list command.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Component("dashboardsectionslist")
public class DashboardSectionsListCommand extends BaseListCommand<ZeroParams> {

    private static final int MAX_AVAILABLE_SECTIONS = 10;

    private List<ListNumberData> list;

    public DashboardSectionsListCommand() {
    ...
    public List<? extends Listable> execute(Locale locale, ZeroParams params) throws UnifyException {
        if (list == null) {
            list = new ArrayList<ListNumberData>();
            for (int i = 1; i <= MAX_AVAILABLE_SECTIONS; i++) {
