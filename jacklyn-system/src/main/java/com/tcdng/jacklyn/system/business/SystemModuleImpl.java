...
/**
 * Default implementation of system business module.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Transactional
@Component(SystemModuleNameConstants.SYSTEMBUSINESSMODULE)
public class SystemModuleImpl extends AbstractJacklynBusinessModule implements SystemModule {
	...
	@Override
	public Long createAuthentication(AuthenticationLargeData authenticationFormData)
	...
	@Override
	public List<Authentication> findAuthentications(AuthenticationQuery query)
			throws UnifyException {
	...
	@Override
	public int updateAuthentication(AuthenticationLargeData authenticationFormData)
			throws UnifyException {
		Authentication authentication = getEncryptedAuthentication(authenticationFormData);
	...
	@Override
	public <T> Map<T, Module> findModules(Class<T> keyClass, String keyName, ModuleQuery query)
			throws UnifyException {
	...
	@Override
	public List<ApplicationMenuItem> findMenuItems(ApplicationMenuItemQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Override
	public List<SystemParameter> findSysParameters(SystemParameterQuery query)
			throws UnifyException {
		return db().listAll(query);
	}

	@Override
	public List<SystemControlState> findSystemControlStates(SystemParameterQuery query)
			throws UnifyException {
		List<SystemControlState> systemControlStateList = new ArrayList<SystemControlState>();
		Criteria criteria = query.getCriteria();
		Query<SystemParameter> innerQuery = query.copyNoCriteria().add(criteria)
				.equals("control", Boolean.TRUE).order("name");
		int index = 0;
		...
		for (SystemParameter sysParameter : list) {
			systemControlStateList.add(new SystemControlState(index++, sysParameter.getName(),
					sysParameter.getDescription(),
					convert(boolean.class, sysParameter.getValue(), null)));
		}
	...
	@Override
	public Long createScheduledTask(ScheduledTaskLargeData scheduledTaskFormData)
			throws UnifyException {
		ScheduledTask scheduledTask = scheduledTaskFormData.getData();
		Long scheduledTaskId = (Long) db().create(scheduledTask);
		getParameterBusinessModule().updateParameterValues(scheduledTask.getTaskName(),
				SCHEDULED_TASK, scheduledTaskId, scheduledTaskFormData.getScheduledTaskParams());
	...
	public ScheduledTaskLargeData findScheduledTaskDocument(Long id) throws UnifyException {
		...
		Inputs parameterValues = getParameterBusinessModule()
				.fetchNormalizedInputs(scheduledTask.getTaskName(), SCHEDULED_TASK, id);
	...
	@Override
	public int updateScheduledTask(ScheduledTaskLargeData scheduledTaskFormData)
			throws UnifyException {
		ScheduledTask scheduledTask = scheduledTaskFormData.getData();
		scheduledTask.setUpdated(Boolean.TRUE);
	...
	public int deleteScheduledTask(Long scheduledTaskId) throws UnifyException {
		String taskName = db().value(String.class, "taskName",
				new ScheduledTaskQuery().id(scheduledTaskId));
		getParameterBusinessModule().deleteParameterValues(taskName, SCHEDULED_TASK,
				scheduledTaskId);
		return db().delete(ScheduledTask.class, scheduledTaskId);
	}

	@Override
	public ScheduledTaskLargeData loadScheduledTaskDocumentValues(
			ScheduledTaskLargeData scheduledTaskFormData) throws UnifyException {
		ScheduledTask scheduledTask = scheduledTaskFormData.getData();
		Inputs parameterValues = getParameterBusinessModule().fetchNormalizedInputs(
				scheduledTask.getTaskName(), SCHEDULED_TASK, scheduledTask.getId());
		return new ScheduledTaskLargeData(scheduledTask, parameterValues);
	}
	...
	@Override
	public List<ScheduledTaskHist> findScheduledTaskHistory(ScheduledTaskHistQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	public List<ToolingRecordTypeItem> findToolingRecordTypes() throws UnifyException {
		...
		for (Class<? extends Entity> entityClass : getAnnotatedClasses(Entity.class,
			...
			resultList.add(
					new ToolingRecordTypeItem(ta.value(), entityClass.getName(), id, fieldList));
		}
	...
	public List<ToolingListTypeItem> findToolingListTypes() throws UnifyException {
		...
		for (Class<? extends EnumConst> enumClass : getAnnotatedClasses(EnumConst.class,
			...
			if (sla != null) {
				...
				resultList.add(new ToolingListTypeItem(sla.value(),
	...
	public void triggerScheduledTasksForExecution(TaskMonitor taskMonitor) throws UnifyException {
		...
		if (!currentDt.equals(workingDt)) {
			for (TaskInfo taskInfo : triggeredTaskInfoMap.values()) {
				if (!taskInfo.isDummy()) {
					logDebug("Stopping scheduled task [{0}] from running...",
			...
			if (now.after(scheduledTaskTime) && isOkToRunOnWorkingDate(scheduledTask)) {
				...
				if (grabClusterLock(taskLock)) {
					...
					try {
						...
						schdParameters.put(SystemSchedTaskConstants.SCHEDULEDTASK_ID,
								scheduledTaskId);
						for (Input parameterValue : getParameterBusinessModule()
								.fetchNormalizedInputs(scheduledTask.getTaskName(), SCHEDULED_TASK,
										scheduledTask.getId())
								.getInputList()) {
							schdParameters.put(parameterValue.getName(),
									parameterValue.getTypeValue());
						}

						TaskMonitor schdTaskMonitor = null;
						if (scheduledTask.getFrequency() != null
								&& scheduledTask.getFrequencyUnit() != null) {
							...
							if (scheduledTask.getExpires()) {
								...
								if (actualWindowToRunMillSec > 0) {
									...
									if (periodInMillSec > 0) {
										int reCalcNumberOfTimes = (int) (actualWindowToRunMillSec
												/ periodInMillSec);
							...
							if (!isExpired) {
								logDebug(
										"Scheduling task [{0}] to run every [{1}ms] with a [{2}ms] delay...",
										scheduledTask.getDescription(), periodInMillSec, 0);
								schdTaskMonitor = taskManager.scheduleTaskToRunPeriodically(
										scheduledTask.getTaskName(), schdParameters, false, 0,
										periodInMillSec, numberOfTimes, taskStatusLogger.getName());
							}
						} else {
							// Check for expiry
							boolean isExpired = false;
							...
							if (!isExpired) {
								// Schedule one-shot
								logDebug("Scheduling task [{0}] to run one time...",
										scheduledTask.getDescription());
								schdTaskMonitor = taskManager.startTask(scheduledTask.getTaskName(),
										schdParameters, false, taskStatusLogger.getName());
							}
						}
						...
						taskStatusLogger.logCriticalFailure(scheduledTask.getTaskName(),
								schdParameters, e);
					}
				}
			}
	...
	public void loadApplicationMenu(String... params) throws UnifyException {
		...
		List<ApplicationMenu> applicationMenuList
				= findMenus((ApplicationMenuQuery) new ApplicationMenuQuery().orderByDisplayOrder()
						.orderByModuleDesc().status(RecordStatus.ACTIVE));
		for (ApplicationMenu applicationMenu : applicationMenuList) {
			List<ApplicationMenuItem> applicationMenuItemList
					= findMenuItems(new ApplicationMenuItemQuery()
							.menuId(applicationMenu.getId()).orderByDisplayOrder());
			List<MenuItem> menuItemList = new ArrayList<MenuItem>();
			for (ApplicationMenuItem applicationMenuItem : applicationMenuItemList) {
				MenuItem menuItem = new MenuItem(applicationMenuItem.getCaption(),
						applicationMenuItem.getName(), applicationMenuItem.getPath(), null);
				menuItemList.add(menuItem);
			}

			menuItemSetList.add(new MenuItemSet(applicationMenu.getCaption(),
					applicationMenu.getName(), applicationMenu.getPath(), menuItemList));
		}

		MenuSet menuSet = new MenuSet();
		menuSet.add(new Menu(getApplicationName(), getApplicationName(),
				Collections.unmodifiableList(menuItemSetList)));
		if (getContainerSetting(boolean.class, UnifyCorePropertyConstants.APPLICATION_OSMODE,
				false)) {
			// Load application menus
			menuSet.setAlwaysSelect(true);
			MenuLoader menuLoader
					= (MenuLoader) getComponent(ApplicationComponents.APPLICATION_MENULOADER);
	...
	public List<Tile> generateTiles(List<DashboardTile> dashboardTileList) throws UnifyException {
		...
		for (DashboardTile dashboardTile : dashboardTileList) {
			if (dashboardTile.getGenerator() != null) {
				DashboardTileGenerator dashboardTileGenerator
						= (DashboardTileGenerator) getComponent(dashboardTile.getGenerator());
				tileList.add(dashboardTileGenerator.generate(dashboardTile));
			} else {
				tileList.add(new Tile(dashboardTile.getImageSrc(), dashboardTile.getCaption(),
						dashboardTile.getPath(), null, dashboardTile.isLandscape()));
			}
		}
		return tileList;
	}
	...
	@Override
	public void saveDashboardTileOrder(List<DashboardTile> dashboardTileList)
			throws UnifyException {
	...
	public void onApplicationStartup() throws UnifyException {
		...
		setApplicationAttribute(JacklynApplicationAttributeConstants.DEFAULT_SYSTEM_USERTOKEN,
				createUserToken(SystemReservedUserConstants.SYSTEM_LOGINID,
						SystemReservedUserConstants.SYSTEM_ID));
		setApplicationAttribute(JacklynApplicationAttributeConstants.DEFAULT_ANONYMOUS_USERTOKEN,
				createUserToken(SystemReservedUserConstants.ANONYMOUS_LOGINID,
	...
	private UserToken createUserToken(String loginId, Long id) throws UnifyException {
		return new UserToken(loginId, "System", getSessionContext().getRemoteAddress(), id, null,
				true, true, false);
	}
	...
	public void installFeatures(List<ModuleConfig> moduleConfigList) throws UnifyException {
		...
		DashboardTileQuery dtQuery
				= (DashboardTileQuery) new DashboardTileQuery().clear().ignoreEmptyCriteria(true);
			...
			if (!moduleConfig.isExtension()) {
				...
				if (nonExtModules.contains(moduleName)) {
					throw new UnifyException(
							SystemModuleErrorConstants.MODULE_MULTIPLE_CONFIGURATIONS_WITH_NAME,
							moduleName);
				}
		...
		for (ModuleConfig moduleConfig : moduleConfigList) {
			...
			if (module == null) {
				throw new UnifyException(
						SystemModuleErrorConstants.MODULE_UNKNOWN_MODULE_NAME_REFERENCED,
						moduleName);
			}

			Long moduleId = module.getId();
			if (moduleConfig.getSysParams() != null) {
				logDebug("Updating system parameter definitions for module [{0}]...",
						module.getDescription());
				for (SysParamConfig sysParamConfig : moduleConfig.getSysParams()
						.getSysParamList()) {
			...
			if (moduleConfig.getMenus() != null) {
				...
				for (MenuConfig menuConfig : menuConfigList) {
					...
					if (oldApplicationMenu == null) {
						logDebug("Registering module menu definition [{0}]...",
								applicationMenu.getName());
						...
						logDebug("Updating module menu definition [{0}]...",
								oldApplicationMenu.getName());
					...
					if (menuItemList != null) {
						...
						for (MenuItemConfig menuItemConfig : menuItemList) {
							...
							ApplicationMenuItem oldApplicationMenuItem
									= db().find(miQuery.name(menuItemName));
							if (oldApplicationMenuItem == null) {
								...
								oldApplicationMenuItem
										.setDescription(menuItemConfig.getDescription());
								oldApplicationMenuItem
										.setPageCaption(menuItemConfig.getPageCaption());
								oldApplicationMenuItem.setCaption(menuItemConfig.getCaption());
								oldApplicationMenuItem.setPath(menuItemConfig.getPath());
								oldApplicationMenuItem
										.setRemotePath(menuItemConfig.getRemotePath());
			...
			if (moduleConfig.getDashboardTiles() != null) {
				...
				for (DashboardTileConfig dashboardTileConfig : moduleConfig.getDashboardTiles()
					...
					dtQuery.clear();
					DashboardTile oldDashboardTile
							= db().find(dtQuery.name(dashboardTileConfig.getName()));

					if (oldDashboardTile == null) {
						dashboardTile.setName(dashboardTileConfig.getName());
						dashboardTile.setDescription(
								resolveApplicationMessage(dashboardTileConfig.getDescription()));
						dashboardTile.setImageSrc(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getImage()));
						dashboardTile.setCaption(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getCaption()));
						dashboardTile.setGenerator(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getGenerator()));
						...
						oldDashboardTile.setDescription(
								resolveApplicationMessage(dashboardTileConfig.getDescription()));
						oldDashboardTile.setImageSrc(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getImage()));
						oldDashboardTile.setCaption(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getCaption()));
						oldDashboardTile.setGenerator(AnnotationUtils
								.getAnnotationString(dashboardTileConfig.getGenerator()));
			...
			if (moduleConfig.getInputControls() != null) {
				...
				for (InputControlConfig icc : moduleConfig.getInputControls()
						.getInputControlList()) {
					InputCtrlDef oldInputCtrlDef
							= db().find(new InputCtrlDefQuery().name(icc.getName()));
		...
		// Define parameters for all schedulable tasks and business module tasks
		...
		for (TaskableMethodConfig bmtc : taskManager.getAllTaskableMethodConfigs()) {
			if (bmtc.isSchedulable()) {
				...
				getParameterBusinessModule().defineParameters(bmtc.getName(), parameterList);
			}
		}
		...
		for (Class<? extends RemoteCallController> remoteCallClass : this
			...
			for (Method method : methods) {
				...
				if (goa != null) {
					sysAssetQuery.clear();
					SystemAsset oldSystemAsset = db().find(
							sysAssetQuery.type(SystemAssetType.REMOTECALLMETHOD).name(goa.name()));
	...
	private AuthenticationLargeData internalFindAuthentication(Authentication authentication)
			throws UnifyException {
		TwoWayStringCryptograph cryptograph
				= (TwoWayStringCryptograph) getComponent(authentication.getCryptograph());
	...
	private Authentication getEncryptedAuthentication(
			AuthenticationLargeData authenticationFormData) throws UnifyException {
	...
	private void updateModuleStatus(String name, RecordStatus status) throws UnifyException {
		...
		if (!db().value(boolean.class, "deactivatable", query)) {
			throw new UnifyException(SystemModuleErrorConstants.MODULE_DATA_NOT_DEACTIVATABLE,
					name);
		}
	...
	private boolean isOkToRunOnWorkingDate(ScheduledTask scheduledTask) throws UnifyException {
		...
		return RecordStatus.ACTIVE.equals(scheduledTask.getStatus())
				&& CalendarUtils.isWithinCalendar(scheduledTask.getWeekdays(),
						scheduledTask.getDays(), scheduledTask.getMonths(), workingDt);
	}

	private List<ScheduledTask> listNewScheduledTasks(Date time, List<Long> oldScheduledTaskIds)
			throws UnifyException {
