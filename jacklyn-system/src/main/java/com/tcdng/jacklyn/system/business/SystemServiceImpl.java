...
public class SystemServiceImpl extends AbstractJacklynBusinessService implements SystemService {
		...
		scheduledTaskDefs = new FactoryMap<Long, ScheduledTaskDef>(true) {
			...
			protected ScheduledTaskDef create(Long scheduledTaskId, Object... params) throws Exception {
				...
				if (scheduledTask.getFrequency() != null && scheduledTask.getFrequencyUnit() != null) {
					repeatMillSecs = CalendarUtils.getMilliSecondsByFrequency(scheduledTask.getFrequencyUnit(),
							scheduledTask.getFrequency());
				}

				List<Input<?>> inputList = getParameterService()
		...
		dashboardDefs = new FactoryMap<String, DashboardDef>(true) {
			...
			protected DashboardDef create(String name, Object... params) throws Exception {
				...
				for (DashboardPortlet dashboardPortlet : dashboard.getPortletList()) {
					...
					long refreshMillSec = dashboardPortlet.getRefreshPeriod() != null ? CalendarUtils
							.getMilliSecondsByFrequency(FrequencyUnit.SECOND, dashboardPortlet.getRefreshPeriod()) : 0L;
				...
				String dashboardViewer = UplUtils.generateUplGeneratorTargetName("dashboard-generator",
						dashboard.getName());
	...
	public List<SystemControlState> findSystemControlStates(SystemParameterQuery query) throws UnifyException {
		...
		Query<SystemParameter> innerQuery = query.copyNoCriteria().addRestriction(criteria)
				.addEquals("control", Boolean.TRUE).addOrder("name");
		int index = 0;
	...
	public boolean executeTestDataSourceTask(TaskMonitor taskMonitor, DataSource dataSource) throws UnifyException {
		...
		result = dataSourceManager.testConfiguration(new DynamicSqlDataSourceConfig(taskMonitor.getTaskId(0),
				driver.getDialect(), driver.getDriverType(), dataSource.getConnectionUrl(), dataSource.getSchema(),
	...
	public void loadApplicationMenu(String... params) throws UnifyException {
		...
		List<ApplicationMenu> applicationMenuList = findMenus((ApplicationMenuQuery) new ApplicationMenuQuery()
				.orderByDisplayOrder().orderByModuleDesc().installed(Boolean.TRUE).status(RecordStatus.ACTIVE));
				...
				for (SysParamConfig sysParamConfig : moduleConfig.getSysParams().getSysParamList()) {
					...
					if (sysParameter == null) {
						...
						if (SystemModuleSysParamConstants.SYSPARAM_SYSTEM_EMAIL.equals(sysParamConfig.getName())) {
							defaultVal = getContainerSetting(String.class,
			...
			for (Method method : methods) {
				...
				if (goa != null) {
					...
					SystemAsset oldSystemAsset = db()
							.find(sysAssetQuery.type(SystemAssetType.REMOTECALL_METHOD).name(goa.name()));
	...
	private UserToken createReservedUserToken(String loginId, Long id) throws UnifyException {
		...
		return UserToken.newBuilder().userLoginId(loginId).userName("System")
				.ipAddress(getSessionContext().getRemoteAddress()).branchCode(branchCode).zoneCode(zoneCode)
				.tenantCode(tenantCode).colorScheme(colorScheme).globalAccess(true).reservedUser(true)
				.allowMultipleLogin(true).build();
	...
	private DynamicSqlDataSourceConfig getDynamicSqlDataSourceConfig(DataSource dataSource) throws UnifyException {
		return new DynamicSqlDataSourceConfig(dataSource.getPreferredName(), dataSource.getName(),
				dataSource.getDialect(), dataSource.getDriverType(), dataSource.getConnectionUrl(),
				dataSource.getSchema(), dataSource.getUserName(), dataSource.getPassword(),
