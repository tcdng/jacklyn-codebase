...
/**
 * Default implementation of audit business module.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Transactional
@Component(AuditModuleNameConstants.AUDITBUSINESSMODULE)
public class AuditModuleImpl extends AbstractJacklynBusinessModule implements AuditModule {
	...
	@Override
	public int setAuditTypeStatus(AuditDefinitionQuery query, RecordStatus status)
	...
	@Override
	public InspectUserInfo fetchInspectUserInfo(String userLoginId, Date createDt, Long moduleId,
			EventType eventType) throws UnifyException {
		if (StringUtils.isBlank(userLoginId)) {
			return new InspectUserInfo();
		}

		User user = securityModule.findUser(userLoginId);
		byte[] photo = securityModule.findUserPhotograph(user.getId());
		...
		if (!auditTrailList.isEmpty()) {
			...
			for (AuditTrail auditTrail : auditTrailList) {
				...
				auditItems.add(new InspectUserAuditItem(auditTrail.getAuditDesc(),
						auditTrail.getModuleDesc(), auditTrail.getIpAddress(),
						auditTrail.getCreateDt(), auditTrail.getEventType(),
	...
	public void installFeatures(List<ModuleConfig> moduleConfigList) throws UnifyException {
		...
		db().updateAll(new AuditDefinitionQuery(),
			...
			if (moduleConfig.getAudits() != null) {
				...
				for (AuditConfig auditConfig : moduleConfig.getAudits().getAuditList()) {
					...
					if (auditConfig.isType()) {
						auditTypeId
								= installAuditType(JacklynUtils.getManagedConfig(moduleConfig,
										auditConfig.getAuditable()), auditConfig);
					}

					AuditDefinition oldAuditDefinition
							= db().find(adQuery.name(auditConfig.getName()));
					String description
							= resolveApplicationMessage(auditConfig.getDescription());
	...
	private Long installAuditType(ManagedConfig managedConfig, AuditConfig auditConfig)
			throws UnifyException {
		Long auditTypeId = null;
