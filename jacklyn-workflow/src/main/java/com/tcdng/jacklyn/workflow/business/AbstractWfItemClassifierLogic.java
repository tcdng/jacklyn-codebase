 ...
 * @since 1.0
 */
public abstract class AbstractWfItemClassifierLogic extends AbstractUnifyComponent
		implements WfItemClassifierLogic {
	 ...
	 *             if an error occurs
	 */
	protected boolean applyFilter(WfItemReader wfItemReader, WfDocClassifierFilterDef filter)
			throws UnifyException {
		if (wfItemReader != null) {
			...
			switch (filter.getOp()) {
				case BETWEEN:
					if (fieldValue != null) {
						...
						return dblFieldVal >= ((Number) limVal1).doubleValue()
								&& dblFieldVal <= ((Number) getCalcValue(wfItemReader, filter,
										filter.getValue2())).doubleValue();
					}
					break;
				case EQUALS:
				...
				case GREATER:
					if (fieldValue != null) {
						return ((Number) fieldValue).doubleValue() > ((Number) limVal1)
								.doubleValue();
					}
					break;
	...
	private Object getCalcValue(WfItemReader wfItemReader, WfDocClassifierFilterDef filter,
			String value) throws UnifyException {
		Object val = null;
