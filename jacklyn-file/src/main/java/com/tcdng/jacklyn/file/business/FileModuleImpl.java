...
public class FileModuleImpl extends AbstractJacklynBusinessModule implements FileModule {
	...
	@Override
	public List<FileTransferConfig> findFileTransferConfigs(FileTransferConfigQuery query)
	...
	@Override
	public Long createFileTransferConfig(FileTransferConfig fileTransferConfig)
			throws UnifyException {
		return (Long) db().create(fileTransferConfig);
	}

	@Override
	public int updateFileTransferConfig(FileTransferConfig fileTransferConfig)
			throws UnifyException {
	...
	@Override
	public byte[] readRemoteBlock(Long fileTransferConfigId, Date workingDt, String filename,
			long fileIndex, int length) throws UnifyException {
		FileTransferConfig fileTransferConfig
				= db().list(FileTransferConfig.class, fileTransferConfigId);
	...
	public boolean executeTestFileTransferConfigTask(TaskMonitor taskMonitor,
			FileTransferConfig fileTransferConfig) throws UnifyException {
	...
	@Taskable(name = FileTransferTaskConstants.FILETRANSFERLISTUPDATETASK,
			...
			parameters = { @Parameter(name = FileTransferTaskConstants.FILETRANSFERCONFIGNAME,
					...
					editor = "!ui-select list:filetransferconfiglist listKey:name blankOption:$s{}",
					mandatory = true),
					@Parameter(name = FileTransferTaskConstants.WORKINGDT, type = Date.class) },
			schedulable = true)
	public int executeUpdateFileTransferListTask(TaskMonitor taskMonitor,
			String fileTransferConfigName, Date workingDt) throws UnifyException {
		int updatedCount = 0;
		...
		if (FileTransferDirection.UPLOAD.equals(fileTransferConfig.getDirection())) {
			...
			if (!fileInfoList.isEmpty()) {
				...
				for (FileInfo fileInfo : fileInfoList) {
					if (fileInfo.isFile() && !fileInfo.isHidden()
							&& !oldFilenameSet.contains(fileInfo.getFilename())) {
						if (!StringUtils.isBlank(semaphoreSuffix)) {
							File semaphoreFile
									= new File(fileInfo.getAbsolutePath() + semaphoreSuffix);
			...
			if (!fileInfoList.isEmpty()) {
				...
				Set<String> oldFilenameSet = new HashSet<String>(db().valueList(String.class,
						"filename", new FileInboxQuery().setFilenameIn(filenames)));
	...
	@Taskable(name = FileTransferTaskConstants.FILETRANSFERTASK,
			description = "File DataTransfer Task",
			parameters = { @Parameter(name = FileTransferTaskConstants.FILETRANSFERCONFIGNAME,
					description = "$m{file.filetransfertask.parameter.transferconfig}",
					editor = "!ui-select list:filetransferconfiglist listKey:name blankOption:$s{}",
					mandatory = true),
					...
					@Parameter(name = FileTransferTaskConstants.UPDATEFILEBOX,
							...
							editor = "!ui-select list:booleanlist blankOption:$s{}",
							type = boolean.class, mandatory = true) },
			schedulable = true)
	public int executeFileTransferTask(TaskMonitor taskMonitor, String fileTransferConfigName,
			Date workingDt, boolean updateFilebox) throws UnifyException {
		int transferCount = 0;
		if (updateFilebox) {
		...
		if (FileTransferDirection.UPLOAD.equals(fileTransferConfig.getDirection())) {
			List<FileOutbox> fileOutboxList
					= db().listAll(new FileOutboxQuery().status(FileOutboxStatus.NOT_SENT)
							.fileTransferConfigId(fileTransferConfig.getId()).createdOn(workingDt)
							.order("filename"));
			...
			for (FileOutbox fileOutbox : upFileOutboxList) {
				if (!taskMonitor.isCanceled() && uploadFile(taskMonitor, fileServer,
						fileTransferInfo, fileOutbox, semaphoreSuffix)) {
					transferCount++;
				}
			}
			...
			for (FileInbox fileInbox : downFileInboxList) {
				if (!taskMonitor.isCanceled() && downloadFile(taskMonitor, fileServer,
						fileTransferInfo, fileInbox, semaphoreSuffix)) {
					transferCount++;
				}
			}
	...
	@Taskable(name = BatchFileReadTaskConstants.BATCHFILEREADTASK,
			description = "Batch Upload Task",
	...
	public int executeBatchUploadTask(TaskMonitor taskMonitor,
		...
		BatchFileReadConfig batchFileReadConfig
				= getBatchUploadConfig(batchFileReadInputParameters.getName());

		Map<String, Object> inputParameters = getBatchFileReadConfigParameters(batchFileReadConfig);
		inputParameters.putAll(DataUtils
				.getInputHolderNameValueMap(batchFileReadInputParameters.getInputParameterList()));
		...
		BusinessLogicOutput output = executeBusinessLogic(taskMonitor,
				batchFileReadConfig.getReadProcessor(), inputParameters);

		taskMonitor.getCurrentTaskOutput().setResult(
				BatchFileReadProcessorOutputConstants.BATCHFILEREADRESULT, output.getResult(
						Object.class, BatchFileReadProcessorOutputConstants.BATCHFILEREADRESULT));
		return 0;
	}
	...
	public Map<String, Object> getBatchFileReadConfigExecutionParameters(Long batchFileReadConfigId)
			throws UnifyException {
		return getBatchFileReadConfigParameters(
				db().list(BatchFileReadConfig.class, batchFileReadConfigId));
	}

	@Transactional(TransactionAttribute.REQUIRES_NEW)
	public boolean uploadFile(TaskMonitor taskMonitor, FileTransferServer fileServer,
			FileTransferInfo fileTransferInfo, FileOutbox fileOutbox, String semaphoreSuffix)
			throws UnifyException {
		...
		try {
			...
			db().updateAll(new FileOutboxQuery().id(fileOutbox.getId()),
					new Update().add("uploadAttempts", uploadAttempts).add("uploadedOn", new Date())
							.add("status", FileOutboxStatus.SENT));
			return true;
	...
	public boolean downloadFile(TaskMonitor taskMonitor, FileTransferServer fileServer,
			FileTransferInfo fileTransferInfo, FileInbox fileInbox, String semaphoreSuffix)
			throws UnifyException {
		...
		try {
			...
			db().updateAll(new FileInboxQuery().id(fileInbox.getId()),
					new Update().add("downloadAttempts", downloadAttempts)
							.add("downloadedOn", new Date())
	...
	@Override
	public List<BatchFileDefinition> findBatchFileDefinitions(BatchFileDefinitionQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	public BatchFileConfig getBatchConfig(String batchFileDefinitionName) throws UnifyException {
		BatchFileDefinition batchFileDefinition
				= db().find(new BatchFileDefinitionQuery().name(batchFileDefinitionName));
		...
		for (BatchFileFieldDefinition bffda : batchFileDefinition.getFieldDefList()) {
			batchFileConfig.addFieldConfig(bffda.getName(), bffda.getMappedField(),
					bffda.getFormatter(), bffda.getPadDirection(), bffda.getLength(),
					bffda.isTrim(), bffda.isPad(), bffda.isUpdateOnConstraint(),
					bffda.getPadChar());
		}
	...
	@Override
	public List<BatchFileReadConfig> findBatchFileReadConfigs(BatchFileReadConfigQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Override
	public BatchFileReadConfigLargeData findBatchFileReadConfigDocument(Long id)
			throws UnifyException {
		...
		Inputs fileReaderParams = getParameterBusinessModule().fetchNormalizedInputs(
				batchFileReadConfig.getFileReader(), BATCHFILE_READ_CONFIG, id);
	...
	@Override
	public Long createBatchFileReadConfig(BatchFileReadConfigLargeData document)
			throws UnifyException {
		BatchFileReadConfig batchFileReadConfig = document.getData();
		Long batchFileReadConfigId = (Long) db().create(batchFileReadConfig);
		getParameterBusinessModule().updateParameterValues(batchFileReadConfig.getFileReader(),
				BATCHFILE_READ_CONFIG, batchFileReadConfigId, document.getFileReaderParams());
	...
	@Override
	public int updateBatchFileReadConfig(BatchFileReadConfigLargeData document)
			throws UnifyException {
		BatchFileReadConfig batchFileReadConfig = document.getData();
		int updateCount = db().updateByIdVersion(batchFileReadConfig);
		getParameterBusinessModule().updateParameterValues(batchFileReadConfig.getFileReader(),
				BATCHFILE_READ_CONFIG, batchFileReadConfig.getId(), document.getFileReaderParams());
		return updateCount;
	}
	...
	public int deleteBatchFileReadConfig(Long id) throws UnifyException {
		String fileReader
				= db().value(String.class, "fileReader", new BatchFileReadConfigQuery().id(id));
	...
	@Override
	public BatchFileReadConfigLargeData loadBatchFileReadConfigDocumentValues(
			BatchFileReadConfigLargeData document) throws UnifyException {
		BatchFileReadConfig batchFileReadConfig = document.getData();
		Inputs fileReaderParams = getParameterBusinessModule().fetchNormalizedInputs(
				batchFileReadConfig.getFileReader(), BATCHFILE_READ_CONFIG,
				batchFileReadConfig.getId());
		return new BatchFileReadConfigLargeData(batchFileReadConfig, fileReaderParams);
	}
	...
	public BatchFileReadInputParameters getBatchFileReadInputParameters(Long batchUploadConfigId)
			throws UnifyException {
		BatchFileReadConfig batchFileReadConfig
				= db().find(BatchFileReadConfig.class, batchUploadConfigId);
		BatchFileReadInputParameters batchFileReadInputParameters
				= new BatchFileReadInputParameters();
		...
		batchFileReadInputParameters.addInputParameterList(getParameterBusinessModule()
				.fetchInputList(batchFileReadConfig.getReadProcessor()));
	...
	private FileTransferConfig getFileTransferConfig(String name) throws UnifyException {
		...
		if (fileTransferConfig == null) {
			throw new UnifyException(FileModuleErrorConstants.FILETRANSFERCONFIG_NAME_UNKNOWN,
					name);
		}
		return fileTransferConfig;
	}

	private FileTransferInfo getFileTransferInfo(FileTransferConfig fileTransferConfig,
			Date workingDt) throws UnifyException {
		FileTransferPolicy fileTransferPolicy = (FileTransferPolicy) this
				.getComponent(fileTransferConfig.getFileTransferPolicy());
		String serverPath
		...
		return FileTransferInfo.newBuilder().remoteHost(fileTransferConfig.getRemoteHost())
				.remotePort(remotePort)
				.useAuthenticationId(fileTransferConfig.getAuthenticationId())
				.useAuthenticationPassword(fileTransferConfig.getAuthenticationPassword())
				.remotePath(serverPath).localPath(localPath)
				.filterByPrefixes(fileTransferPolicy.getFilePrefixes(workingDt))
				.filterByExtensions(fileTransferPolicy.getFileSuffixes(workingDt))
				.deleteSourceOnTransfer(
						Boolean.TRUE.equals(fileTransferConfig.getDeleteSourceOnTransfer()))
				.build();
	}

	private void setBatchFileFieldDefinitionIndexes(BatchFileDefinition batchFileDefinition)
			throws UnifyException {
		if (batchFileDefinition.getFieldDefList() != null) {
			int i = 0;
			for (BatchFileFieldDefinition batchFileFieldDefinition : batchFileDefinition
					.getFieldDefList()) {
	...
	private BatchFileReadConfig getBatchUploadConfig(String name) throws UnifyException {
		BatchFileReadConfig batchFileReadConfig
				= db().list(new BatchFileReadConfigQuery().name(name));
	...
	private Map<String, Object> getBatchFileReadConfigParameters(
			BatchFileReadConfig batchFileReadConfig) throws UnifyException {
		Map<String, Object> parameters = new HashMap<String, Object>();
		BatchFileConfig batchFileConfig
				= getBatchConfig(batchFileReadConfig.getBatchFileDefinitionName());
		...
		parameters.putAll(getParameterBusinessModule().findParameterTypeValues(
				batchFileReadConfig.getFileReader(), BATCHFILE_READ_CONFIG,
				batchFileReadConfig.getId()));
		return parameters;
	}
