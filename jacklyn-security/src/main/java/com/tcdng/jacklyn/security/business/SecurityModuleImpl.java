...
/**
 * Default implementation of security business module.
 * 
 * @author Lateef Ojulari
 * @since 1.0
 */
@Transactional
@Component(SecurityModuleNameConstants.SECURITYBUSINESSMODULE)
public class SecurityModuleImpl extends AbstractJacklynBusinessModule implements SecurityModule {
	...
	@Override
	public Long registerPrivilegeCategory(String categoryName, String descriptionKey)
	...
	@Override
	public Long registerPrivilege(String categoryName, String moduleName, String privilegeName,
		...
		if (privilegeGroup == null) {
			...
			privilegeGroup.setModuleId(systemModule.getModuleId(moduleName));
		...
		Privilege privilege = db()
				.find(new PrivilegeQuery().privilegeGroupId(privilegeGroupId).name(privilegeName));
		if (privilege == null) {
	...
	@Override
	public boolean updateRegisteredPrivilege(String categoryName, String moduleName,
			String privilegeName, String privilegeDesc) throws UnifyException {
		Privilege privilege = db().find(new PrivilegeQuery().categoryName(categoryName)
				.moduleName(moduleName).name(privilegeName));
		if (privilege != null) {
			privilege.setDescription(privilegeDesc);
	...
	@Override
	public List<PrivilegeCategory> findPrivilegeCategories(PrivilegeCategoryQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Override
	public List<PrivilegeGroup> findPrivilegeGroups(PrivilegeGroupQuery query)
			throws UnifyException {
		return db().listAll(query);
	}
	...
	@Override
	public int setPrivilegeStatuses(List<Long> privilegeIds, RecordStatus status)
			throws UnifyException {
		return db().updateAll(new PrivilegeQuery().idIn(privilegeIds),
				new Update().add("status", status));
	}

	@Override
	public List<RolePrivilegeWidget> findRoleDocumentControls(RolePrivilegeWidgetQuery query)
			throws UnifyException {
		return db().listAll(query);
	}

	@Override
	public int updateRoleDocumentControls(List<RolePrivilegeWidget> rolePrivilegeWidgetList)
			throws UnifyException {
	...
	public int updateRoleWorkflowSteps(Long roleId, List<Long> wfStepIdList) throws UnifyException {
		...
		if (!DataUtils.isBlank(wfStepIdList)) {
			...
			List<WfStep> wfStepList = workflowModule.findSteps(((WfStepQuery) new WfStepQuery()
					.idIn(wfStepIdList).select("wfTemplateId", "name")));
	...
	@Override
	public Long createBiometric(BiometricCategory category, BiometricType type, byte[] image)
			throws UnifyException {
	...
	public int updateUserPhotograph(Long userId, byte[] photograph) throws UnifyException {
		...
		return db().updateAll(new BiometricQuery().id(biometricId),
	...
	public int deleteUser(Long id) throws UnifyException {
		// Delete biometric
		List<Long> biometricIdList
				= db().valueList(Long.class, "biometricId", new UserBiometricQuery().userId(id));
	...
	public byte[] findUserPhotograph(Long userId) throws UnifyException {
		return db().value(byte[].class, "biometric", new UserBiometricQuery().userId(userId)
				.typeName(BiometricType.PHOTOGRAPH).mustMatch(false));
	}
	...
	public void changeUserPassword(String oldPassword, String newPassword) throws UnifyException {
		...
		if (systemModule.getSysParameterValue(boolean.class,
			...
			PasswordHistoryQuery query
					= new PasswordHistoryQuery().userId(userId).password(newPassword);
			...
			if (passwordHistoryIdList.size() >= systemModule.getSysParameterValue(int.class,
	...
	public void setCurrentUserRole(Long userRoleId) throws UnifyException {
		if (userRoleId != null) {
			...
			setSessionAttribute(JacklynSessionAttributeConstants.ROLEDESCRIPTION,
	...
	@Broadcast
	@Override
	public synchronized void loadRoleAttributesToApplication(String... roleNames)
			throws UnifyException {
		if (roleNames.length > 0) {
			for (String roleName : roleNames) {
				// Do document privileges
				Map<String, PrivilegeSettings> docPrivilegeSettings
						= new HashMap<String, PrivilegeSettings>();
				List<Long> rolePrivilegeIdList = db().valueList(Long.class, "id",
						new RolePrivilegeQuery().roleName(roleName)
								.categoryName(PrivilegeCategoryConstants.DOCUMENTCONTROL));
				if (!rolePrivilegeIdList.isEmpty()) {
					List<RolePrivilegeWidget> rolePrivilegeWidgetList = db().listAll(
							new RolePrivilegeWidgetQuery().rolePrivilegeIdIn(rolePrivilegeIdList));
					for (RolePrivilegeWidget rolePrivilegeWidget : rolePrivilegeWidgetList) {
						docPrivilegeSettings.put(rolePrivilegeWidget.getPrivilegeName(),
								new PrivilegeSettings(rolePrivilegeWidget.isVisible(),
										rolePrivilegeWidget.isEditable(),
				...
				Map<String, Set<String>> nonWidgetPrivilegeNames
				...
				setRoleAttributes(role.getName(),
						new RoleAttributes(role.getName(), role.getDescription(),
								docPrivilegeSettings, allAccessWidgetPrivileges,
								nonWidgetPrivilegeNames, wfStepNames));
			}
	...
	public void installFeatures(List<ModuleConfig> moduleConfigList) throws UnifyException {
		...
		registerPrivilegeCategory(PrivilegeCategoryConstants.APPLICATIONUI,
				"reserved.privilegecategory.applicationui");
		registerPrivilegeCategory(PrivilegeCategoryConstants.DASHBOARD,
				"reserved.privilegecategory.dashboard");
		registerPrivilegeCategory(PrivilegeCategoryConstants.DOCUMENTCONTROL,
				"reserved.privilegecategory.documentcontrol");
		registerPrivilegeCategory(PrivilegeCategoryConstants.REPORTABLE,
				"reserved.privilegecategory.reportable");
		...
		for (ModuleConfig moduleConfig : moduleConfigList) {
			if (moduleConfig.getPrivileges() != null) {
				...
				for (PrivilegeGroupConfig privilegeGroupConfig : moduleConfig.getPrivileges()
					...
					privilege.setPrivilegeGroupId(privilegeGroupId);
					for (PrivilegeConfig privilegeConfig : privilegeGroupConfig
							.getPrivilegeList()) {
						pQuery.clear();
						Privilege oldPrivilege = db().find(pQuery.privilegeGroupId(privilegeGroupId)
								.name(privilegeConfig.getName()));
						String description
								= resolveApplicationMessage(privilegeConfig.getDescription());
		...
		if (db().countAll(new UserQuery().id(SystemReservedUserConstants.ANONYMOUS_ID)) == 0) {
			createUser(new User(SystemReservedUserConstants.ANONYMOUS_ID, "Anonymous",
					SystemReservedUserConstants.ANONYMOUS_LOGINID, "info@tcdng.com",
					Boolean.FALSE));
		}
	}
	...
	public void onApplicationStartup() throws UnifyException {
		...
		for (Class<? extends Entity> entityClass : getAnnotatedClasses(Entity.class,
			...
			String reportableName
					= JacklynUtils.generateManagedRecordReportableName(entityClass, title);
	...
	private UserToken createUserToken(User userData) throws UnifyException {
		...
		if (systemModule.getSysParameterValue(boolean.class,
			...
			allowMultipleLogin = systemModule.getSysParameterValue(boolean.class,
	...
	private String generatePassword(User userData, String sysParamNotificationTemplateName)
		...
		if (systemModule.getSysParameterValue(boolean.class,
			...
			String notificationTemplateName = systemModule.getSysParameterValue(String.class,
					sysParamNotificationTemplateName);
			String notificationChannelName = systemModule.getSysParameterValue(String.class,
					SecurityModuleSysParamConstants.SECURITY_EMAIL_CHANNEL);
			String administratorName = systemModule.getSysParameterValue(String.class,
					SecurityModuleSysParamConstants.ADMINISTRATOR_NAME);
			String administratorEmail = systemModule.getSysParameterValue(String.class,
	...
	private String getUserRoleThemeResourcePath(Long userRoleId) throws UnifyException {
		...
		Long themeId = null;
		if (systemModule.getSysParameterValue(boolean.class,
				SecurityModuleSysParamConstants.ENABLE_USER_THEMES)) {
			themeId = userRole.getUserThemeId();
		}

		if (themeId == null && systemModule.getSysParameterValue(boolean.class,
				SecurityModuleSysParamConstants.ENABLE_ROLE_THEMES)) {
			themeId = userRole.getRoleThemeId();
		}

		if (themeId != null) {
			return systemModule.findTheme(themeId).getResourcePath();
		}

		return null;
	}

	private int createRolePrivileges(Long roleId, List<Long> privilegeIdList)
			throws UnifyException {
		// Create new privileges
		...
		for (Long privilegeId : privilegeIdList) {
			...
			String category = db().value(String.class, "categoryName",
					new PrivilegeQuery().id(privilegeId));
	...
	private List<Long> getWfStepIdListForRole(Long roleId) throws UnifyException {
		...
		Set<Long> wfTemplateIds
				= db().valueSet(Long.class, "wfTemplateId", new RoleWfStepQuery().roleId(roleId));
		for (Long wfTemplateId : wfTemplateIds) {
			...
			wfStepIdList.addAll(workflowModule.findStepIds(wfTemplateId, stepNames));
		}
